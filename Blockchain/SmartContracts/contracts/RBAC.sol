...
contract RBAC{
    ...
    /**
    * @notice a role that will be used to group users. 
    * @dev The role id is its position in the roles array. 
    * @param description A description for the role 
    * @param admin The only role that can add or remove 
    * bearers from this role. To have the role bearers to 
    * be also the role admins, you should pass roles.length 
    * as the admin role. 
    ...
    * @return The role id. 
     */
     function addRole(string memory _roleDescription, uint256 _admin)
        ...
        returns (uint256)
    {
        require(_admin <= roles.lenght, "Admin role does not exist"); 
    ...
    * @return Whether the account is a bearer of the role. 
     */
     function hasRole(address _account, uint256 role)
        public 
        view
        returns (bool)
    {
    ...
    /**
    * @notice A method to add a bearer to the role. 
    ...
    * @param _role The role to remove the bearer from. 
        ...
        public 
    {
        ...
        require(
            hasRole(msg.sender, roles[_role].admin), 
            "User cannot remove bearers"
        ); 
        require(
            hasRole(_account, _role), 
            "Account is not bearer of role"
        );
        delete roles[role].bearers[_account]; 
...
/**
    ...
    - addBearer adds a bearer to the role 
    - removeBearer reverts on non exiting roles 
